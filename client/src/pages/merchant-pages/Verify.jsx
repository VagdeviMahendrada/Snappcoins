...
const Verify = () => {
  ...
  const { id, email,forgotpassword } = location.state;
  ...
  const handleVerifyOtp = async (e) => {
    ...
    const config = {
      url: "/auth/verifyotp",
      method: "post",
      params: { uid: id, otp: finalOtp },
    };
    fetchData(config)
      .then(() => {
        console.log(forgotpassword)
        if (forgotpassword){
          navigate('/merchant-forgotpassword',{ state: { id } })
        }
        else{
          localStorage.removeItem("verify");
          token ? navigate("/merchant-dashboard") : navigate("/merchant-login");
        }
      })
      .catch((err) => {
        console.log(err);
        navigate("/merchant-verify", { state: { id, email } });
      });
    setOtp("");
    setVisibility(false);
  };
